...
- [x] C.2 Create store slices: `catalog`, `cart`, `user`, `navigation` — ✅ done (2026-08-29; `shell/src/app/store/` with per-slice files `{catalog,cart,user,navigation}.{actions,reducer,selectors}.ts` (12 files) + `shell.effects.ts`; 4 slices with typed state interfaces: `catalog` {products, categories}, `cart` {cart}, `user` {user}, `navigation` {current, history})
- [x] C.3 Create `ShellEffects` with functional `createEffect` for catalog/cart/user loads — ✅ done (2026-08-29; `shell.effects.ts`; cart effect sequences the cross-slice user→cart dependency (waits for the user's `loaded` flag before fetching the cart); Apollo Client 4.x Promise→Observable wrapping via rxjs `from()`)
- [x] C.4 Wire `provideStore`, `provideEffects`, `provideRouterStore` into `app.config.ts` — ✅ done (2026-08-29; `provideStore({ catalog, cart, user, navigation, router: routerReducer })`, `provideEffects([ShellEffects])`, `provideRouterStore()`)
...
> **Context (2026-08-29):** The single-stage Docker images shipped the full `node_modules` tree into the runtime layer — `mf/shell:latest` was **2.3GB** and each of the 4 backend services was **~2.83GB**. The user asked: *"Could the projects install the dependencies at build time instead of uploading the node_modules? This should reduce the image right?"* — yes. The fix is multi-stage builds: install + build in a throwaway build stage, and copy only the runtime artifacts into a slim runtime stage.

- [x] D.1 Multi-stage `Dockerfile.shell` — ✅ done (2026-08-29; build stage runs `pnpm install` + `ng build`; runtime stage copies **only** `dist/shell/` (the fully-bundled `server/server.mjs` + `browser/` assets) — zero `node_modules` in the final image; **333MB** (from 2.3GB, ~85% reduction); BUILD + RUNTIME VERIFIED — `node dist/shell/server/server.mjs` starts and serves SSR with no `node_modules` present)
- [x] D.2 Multi-stage backend Dockerfiles (gateway, catalog-svc, cart-svc, user-svc) — ✅ done (2026-08-29; build stage runs `pnpm install` + `cd server-shared && pnpm prisma:generate` + `tsc`; deploy stage runs `pnpm deploy --legacy -P` (production-only, no devDeps) + Prisma client copy; **~750MB** each (from ~2.83GB, ~73% reduction); all 4 import `@jrumandal/shared` → all 4 need the generated Prisma client in the deploy target)
- [x] D.3 Fix Prisma client stub/nesting bug — ✅ done (2026-08-29; two sub-bugs: (A) `pnpm deploy` carries the registry **stub** `.prisma/client`, so a plain `cp -r` **nested** the real client inside the stub instead of replacing it; (B) `rm -rf` removed the parent dir but `cp -r` does not create intermediate dirs. Fix block: locate the real client by `*.so.node` presence, `rm -rf "$NM/.prisma"` + `mkdir -p "$NM/.prisma"` + `cp -r "$SRC" "$NM/.prisma/client"`; verified `default.js`=182 bytes (real re-export), no nested dir, engine `.so.node` present)
- [x] D.4 Verify: all 7 services Healthy + runtime smoke tests — ✅ done (2026-08-29; `postgres`, `pgadmin`, `catalog-svc`, `cart-svc`, `user-svc`, `gateway`, `shell` all Healthy; gateway GraphQL query returns demo data (user `cmta279rp000muic0kagocyyz`, cart 3 items / 36997 USD, 4 products / 3 categories); service `/health` endpoints 200; shell SSR returns 200)
- [x] D.5 Documentation: `mf-orchestrator/README.md` (Docker deployment section) + `plan.md` (Phase D) + `STEPS.md` (this section) — ✅ done (2026-08-29; all 3 docs updated)
