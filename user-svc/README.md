...
The service ships as a **multi-stage** image (`mf/user-svc:latest`, ~750 MB, down from ~2.83 GB single-stage). Dependencies are installed **at build time**; only the compiled `dist/` output plus a production `node_modules` (via `pnpm deploy --legacy -P`) is shipped to the runtime stage.

- **Build stage** — `node:22-slim` + pnpm: `pnpm install` (workspace), `cd server-shared && pnpm prisma:generate`, then `tsc` build.
- **Runtime stage** — `node:22-slim`; copies the `pnpm deploy` target (prod deps only) and the generated Prisma client, then runs `node dist/main.js` (GraphQL on `PORT`, default 4003 in compose).
- **Build command** — `docker build -f Dockerfile.user-svc -t mf/user-svc:latest .` (from the compose context), then `docker compose up -d`.

> The same multi-stage pattern applies to the other backends (`gateway`, `catalog-svc`, `cart-svc`). See `mf-orchestrator/README.md` → *Docker deployment* for the full service table, image sizes, and gotchas (Prisma client stub vs. real, layer caching).
