 ...
 *
 * Renders the `<catalog-mf>` component to an HTML string on the server (Node).
 * The shell (Phase 3) calls `render(props)` and inlines the returned markup.
 *
 ...
 * @returns A promise that resolves to the serialized inner HTML of `<catalog-mf>`.
 */
export async function render(props: CatalogProps = {}): Promise<string> {
  ...
  const doc =
    '<!doctype html><html><head></head><body><catalog-mf></catalog-mf></body></html>';
  const html = await renderApplication(bootstrap, { document: doc, url: '/catalog' });

  // Extract the inner HTML of the <catalog-mf> element so the shell can inline it.
  const match = html.match(/<catalog-mf[^>]*>([\s\S]*?)<\/catalog-mf>/);
