...
export interface SharedApolloClientOptions {
  /** The GraphQL endpoint to query — the gateway (e.g. `http://localhost:4200/graphql`). */
  uri: string;
  /** Additional HTTP headers sent with every request (e.g. auth tokens). */
  headers?: Record<string, string>;
  /** `fetch` credentials mode for cross-origin gateway requests. Defaults to `same-origin`. */
